...
def plotly_plot_forecast(df_actuals, df_forecast, display_fig=True, figsize=(1400, 500), **kwargs):
...
def plotly_plot_backtest(df_backtest, df_forecast=None, display_fig=True, figsize=(1400, 500), **kwargs):
    ...
    if isinstance(df_forecast, pd.DataFrame):
        ...
        df_subline = df_forecast.dropna()
        df_subline.columns = ["actual", "lower", "upper"]
        df_subline = df_backtest.drop(["forecast"], axis=1).iloc[-1:].append(df_subline.iloc[:1])
        ...
        df_subline = df_backtest.drop(["actual"], axis=1).iloc[-1:].append(df_subline.iloc[:1])
...
def plotly_feature_importance(df_features, number_features=10, display_fig=True, figsize=(1400, 300), **kwargs):
...
def plotly_plot_simulation(df_actuals, df_forecast, df_simulation, conf_intervall=True, display_fig=True, figsize=(1400, 500), **kwargs):
    """ Function to generate the forecast plot with actuals and confidence intervall.
    ...
    if conf_intervall:
        ...
        fig.add_trace(
            go.Scatter(
                x=np.concatenate(
                    [
                        df_test_simulation.index, 
                        df_test_simulation.index[::-1]
                        ]
                    ),
                y=np.concatenate(
                    [
                        df_test_simulation["upper"], 
                        df_test_simulation["lower"][::-1]
                        ]
                    ),
                name="Simulation CI*",
                fill='toself',
