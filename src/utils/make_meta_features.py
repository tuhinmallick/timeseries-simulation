...
def create_meta_feature_frame(
    df, horizon, lead=24, rolling_periods=None, ci_alpha=None
):
    if not df.index.freq:
        raise ValueError(
            "Series must have freq attribute set for confidence interval prediction"
        )
    freq = df.index.freq
    ci_lower_list = []
    ci_upper_list = []
    ...
    for last_train_date in tqdm(df.index[lead:], desc="Making meta features"):
        ...
        if rolling_periods is None:
            ...
            fit_data = df.loc[
                last_train_date - rolling_periods * freq : last_train_date
            ]
        else:
            raise ValueError(
                "Argument rolling_periods must be either None or a positive integer!"
            )
        model = ETSModel(
            fit_data,
            trend="add",
        ...
        f = results.predict(
            start=last_train_date + 1 * freq, end=last_train_date + horizon * freq
        )
        metalist.append(
            pd.DataFrame(
                data=f.to_numpy().reshape((1, horizon)),
                index=pd.Index([pd.Timestamp(last_train_date)]),
                columns=[f"{meta_feature_name}_mean_t{t + 1}" for t in range(horizon)],
            )
        )
        if ci_alpha is not None:
            cis = results.get_prediction(
                start=last_train_date + 1 * freq, end=last_train_date + horizon * freq
            )
            cis = cis.pred_int(alpha=ci_alpha)
            ci_lower = cis.iloc[:, 0]
            ci_lower_list.append(
                pd.DataFrame(
                    data=ci_lower.to_numpy().reshape((1, horizon)),
                    index=pd.Index([pd.Timestamp(last_train_date)]),
                    columns=[
                        f"{meta_feature_name}_lower_t{t+1}" for t in range(horizon)
                    ],
                )
            )
            ci_upper = cis.iloc[:, 1]
            ci_upper_list.append(
                pd.DataFrame(
                    data=ci_upper.to_numpy().reshape((1, horizon)),
                    index=pd.Index([pd.Timestamp(last_train_date)]),
                    columns=[
                        f"{meta_feature_name}_upper_t{t+1}" for t in range(horizon)
                    ],
                )
            )
...
if __name__ == "__main__":
    ...
    target = "WPU101704"
    df = pd.read_csv(
        r"../data/input/steel_w_indicators_expanded.csv", index_col=0, parse_dates=True
    )
    df = df[target].dropna()
    df.index.freq = "MS"
    horizon = 3
