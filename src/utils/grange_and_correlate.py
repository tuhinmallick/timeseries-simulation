...
def calculate_grangercausality(target, feature, maxlag=12):
    ...
    for lag in lag_range:
        res = stattools.grangercausalitytests(pd.DataFrame(target.dropna()).join(feature.dropna(), how="inner"), maxlag=maxlag, verbose=False)
...
def grange_and_correlate(df, target, granger_lags=12, correlation_lags=12, number_of_features=10):
    ...
    for i in range(0, len(feature_names)):
        ...
        f_list, p_list = calculate_grangercausality(df[target], df[feature_names[i]], maxlag=granger_lags)
    ...
    granger_features = df_granger.sort_values("fval", ascending=False).reset_index(drop=True).loc[:number_of_features, "feature_name"].tolist()
    ...
    for i in range(0, len(feature_names)):
        ...
        xcov_monthly = [crosscorr(df[target], df[feature_names[i]], lag=lag) for lag in range(-correlation_lags, correlation_lags)]
        ...
        df_eval = pd.DataFrame(
            {
                ...
                "abs_corr": xcov_monthly_abs
                }
        )

        df_eval_filtered = df_eval.sort_values("abs_corr").reset_index(drop=True).iloc[:1]
        df_eval_filtered["feature_name"] = feature_names[i]

        df_corr = df_corr.append(df_eval_filtered)
    df_corr.reset_index(drop=True, inplace=True)
    # Abstract desired correlation features
    corr_features = df_corr.sort_values("abs_corr", ascending=False).reset_index(drop=True).loc[:number_of_features, "feature_name"].tolist()
    
    # Combine the relevant features in list and get unique
    relevant_features = [item for sublist in zip(granger_features, corr_features) for item in sublist]
...
def user_input_crosscorr(df_target, df_user_input, max_lags=30):
    ...
    for i in range(0, len(feature_names)):
        # Apply cross correlation function
        xcov_monthly = [crosscorr(df_target, df_user_input[feature_names[i]], lag=lag) for lag in range(-max_lags, max_lags)]
...
def user_input_correlation_picker(df_target, df_user_input, selected_method="mean", max_lags=12):
    ...
    crosscorr_dict = user_input_crosscorr(df_target=df_target, df_user_input=df_user_input, max_lags=max_lags)
    feature_names = df_user_input.columns.tolist()
