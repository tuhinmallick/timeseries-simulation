...
def cross_val_forecast_dates_darts(
    model,
    series,
    dates_list,
    horizon,
    ...
    for last_train_date in dates_list:
        ...
        forecast_series = darts_date_predictions(
            series,
            model,
            last_train_date,
            horizon,
...
def step_results_frame(
    metrics_dict: dict, column: str, table="test", print_results=False
):
    store_list = []
...
def plot_backtest_forecasts(
    y_full, forecast_table_list, figsize=(14, 12), plot_before=24, shift=False
):
    """
    ...
    for i in range(3):
        ...
        if shift == True:
            ...
            start_date = forecast_table.index[
                0
            ]  # need to adjust start date after shifting
        else:
        ...
        y_aligned, backtest_forecasts = y_full.align(
            backtest_forecasts, join="inner", axis=0
        )
        # rmse_score = rmse_func(y_full.reindex_like(backtest_forecasts).to_numpy(), backtest_forecasts.to_numpy())
        mape_val = mape(
            y_full.reindex_like(backtest_forecasts).to_numpy(),
            backtest_forecasts.to_numpy(),
        )
...
def plot_forecast(metrics_dict, y_full, start_date, start_date_text, ax=None):
    ...
    xaxis_start = max(
        pd.Timestamp(start_date) - plot_before * frequency, y_full.index.min()
    )
    ...
    props = dict(boxstyle="round", facecolor="black", alpha=0.5)
    _ = ax.text(
        x=0.02, y=0.5, s=f"{metric_text}{score:.2f}", transform=ax.transAxes, bbox=props
    )
    _ = ax.set_title(f"Forecast after {start_date_text}")
