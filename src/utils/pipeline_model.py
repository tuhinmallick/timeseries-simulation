...
class PipelineModel():
    ...
    @staticmethod
    def get_gt_pred_data(y_tra, tra_pred, y_tes, tes_pred, gt, horizon, targets, out_pred=None, ci=None):
        # Generate gt + prediction df file
        tra_pred = pd.DataFrame(tra_pred, columns=targets + "_pred", index=y_tra.index.shift(horizon))
        tes_pred = pd.DataFrame(tes_pred, columns=targets + "_pred", index=y_tes.index.shift(horizon))
        outputdf = pd.concat([gt[targets], tra_pred.append(tes_pred)], axis=1)
        if out_pred is not None:
            out_pred = pd.DataFrame(
                np.array(out_pred), columns=targets + "_pred", index=outputdf.index[-horizon:].shift(horizon)
            )
    ...
    def get_quantile_regression_pred_interval(self, X_tra, y_tra, X_tes, y_tes=None, low=.05, high=.95):
        ...
        for quan in [low, high]:
            ...
            tmp_tuple = PipelineModel.get_model(hps, self.estimator, self.multiout_wrapper, **self.kwargs).steps[-1]
    ...
    def get_bootstrap_pred_interval(
          self, X, y, n_samples: int, train_size: int, block_length: int, bootstrap_type: str = 'cbb', low=.05,
          high=.95, seed: int = 42, model_mode='last_step_tune'
    ):
        assert X.ndim <= 2
        assert y.ndim <= 2
        ...
        bts = BootstrappingWrapper(
            ...
            bootstrap_type=bootstrap_type, block_length=block_length
        )
        ...
        for i in range(data.shape[1]):
            ...
            tmp = data[:, i]
            mask = np.isnan(tmp)
            tmp[mask] = np.random.normal(tmp[~mask].mean(), tmp[~mask].std(), size=mask.sum())
            ## sampling
        ...
        with warnings.catch_warnings(record=False) as cw:
            ...
            for i in tqdm(range(X_tra.shape[0])):
                if model_mode == 'all_tune':
                    model = PipelineModel.get_model(self.hps, self.estimator, self.multiout_wrapper, **self.kwargs)
                    model.fit(X_tra[i], y_tra[i])
                elif model_mode == 'last_step_tune':
                    model = Pipeline(steps=self.model.steps[0:-1])
                    tmp_tuple = \
                        PipelineModel.get_model(self.hps, self.estimator, self.multiout_wrapper, **self.kwargs).steps[
                            -1]
                    # fit quantile model only on last step 
                    tmp = X_tra[i]
        ...
        idl, idm, idu = int(pred.shape[0] * low), pred.shape[0] // 2, int(pred.shape[0] * high)
        ci_lower, median, ci_upper = pred[idl], pred[idm], pred[idu]
        output = {"ci_lower": ci_lower, "median": median, "ci_upper": ci_upper, "bootstrapped_models": model_li}
        return output

    def get_feature_importance(self, in_feature_names, targets_names=[], normalize=False):
    ...
    def binary_performances(y_true, y_prob, thresh=0.5, labels=['Positives', 'Negatives']):
        ...
        plt.subplot(131)
        ax = sns.heatmap(
            cm, annot=True, cmap='Blues', cbar=False,
        ...
        plt.subplot(132)
        plt.hist(
            y_prob[y_true == 1], density=True, bins=25,
            alpha=.5, color='green', label=labels[0]
        )
        plt.hist(
            y_prob[y_true == 0], density=True, bins=25,
        ...
        plt.subplot(133)
        plt.plot(
            fp_rates, tp_rates, color='orange',
