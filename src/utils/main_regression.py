...
pipelib_path = (
    r"C:\Users\SutcliG\source\commodity-desk-modeling\models\SmoothBoost\pipelib"
)
...
if __name__ == "__main__":
    ...
    if os.path.exists("outputs"):
        shutil.rmtree("outputs")  # Delete any previous artifacts
    os.makedirs("outputs")
    ...
    df = pd.read_csv(
        r"../data/input/steel_w_indicators_expanded.csv",
        index_col=0,
        parse_dates=True,
        encoding="latin-1",
    )
    df.index.freq = "MS"
    df = df[
        [target_name] + ["PCOALAUUSDM", "A31ANO", "A31ATI", "WPU1012", "VIX_monthly"]
    ]
    ...
    meta_features = create_meta_feature_frame(
        df[target_name].dropna(), horizon, ci_alpha=0.05
    )
    ...
    fig.savefig(
        "outputs/backtest/barplot_rmse_backtest.svg",
    ...
    fig.savefig(
        "outputs/backtest/backtest_forecasts_plot.svg",
        facecolor=fig.get_facecolor(),
        edgecolor="none",
    )
    #
    ...
    forecasts_dict = forecast_dates(
        model, df, target_series, test_dates, meta_features=meta_features
    )
    # Save the forecasts to an excel spreadsheet
    metrics_dict = fu.forecast_metrics_dict(forecasts_dict, target_series)
    fu.results_xlsx_from_metrics_dict(
        metrics_dict,
        [
    ...
    sns.despine()
    fig.savefig(
        "outputs/val_dates/barplot_mae_dates.svg",
        facecolor=fig.get_facecolor(),
        edgecolor="none",
    )
    ...
    sns.despine()
    fig.savefig(
        "outputs/val_dates/barplot_rmse_dates.svg",
        facecolor=fig.get_facecolor(),
        edgecolor="none",
    )
    ...
    for d in test_dates:
        ...
        try:
            ...
            _ = fig.savefig(
                f"outputs/val_dates/forecast_plots/forecast_{d}.svg",
                facecolor=fig.get_facecolor(),
                edgecolor="none",
            )
            plt.close()
    ...
    forecast.loc[target_series.index.max(), :] = target_series.loc[
        target_series.index.max()
    ][0]
    ...
    _ = fig.savefig(
        "outputs/final_forecast.svg", facecolor=fig.get_facecolor(), edgecolor=None
    )
    ...
    for i in range(horizon):
        feature_importances = model.forecasters[i].regressor.get_feature_importance(
    ...
    # Save scripts
    shutil.rmtree(
        os.path.join(pipelib_path, "__pycache__"), ignore_errors=True
    )  # Delete this to save space
    shutil.rmtree(
        os.path.join(pipelib_path, "kaisutils/__pycache__"), ignore_errors=True
    )  # Delete this to save space
