...
def forecaster_cross_val_score(
      regressor, X, target_series, timestep, cv, forecast_type, meta_feature_prefix=None,
      meta_features=None
):
    ...
    for train_idx, test_idx in cv.split(X):
        ...
        ssf = rf.SingleTimestepRegressionForecaster(
            regressor, timestep=timestep, forecast_type=forecast_type,
    ...
    rmse_overall = sklearn.metrics.mean_squared_error(y_aligned, y_pred_all, squared=False)
    return rmse_overall


def test_hyperparams(
      regressor, timestep, X, target_series, cv, forecast_type, meta_feature_prefix=None,
      meta_features=None, hps=None
):
    ...
    """
    pm = pipeline_model.PipelineModel(hps=hps, estimator=regressor, multiout_wrapper=False)
    rmse = forecaster_cross_val_score(
        pm, X, target_series, timestep, cv, forecast_type, meta_feature_prefix,
        meta_features
    )
    return rmse
...
def hyperopt_all_timesteps(
      regressor, horizon, X, target_series, space, cv, max_evals, forecast_type,
    ...
    for timestep in range(1, horizon + 1, 1):
        objective = functools.partial(
            test_hyperparams, regressor, timestep, X, target_series, cv, forecast_type,
            meta_feature_prefix, meta_features
        )
        best = hyperopt.fmin(objective, space, algo=hyperopt.tpe.suggest, max_evals=max_evals)
...
def wmape_error(y_true, y_pred):  # From the commodity desk utils folder
    wmape = np.sum(np.abs(y_pred.to_numpy() - y_true.to_numpy())) / np.sum(np.abs(y_true.to_numpy()))
    return wmape
...
def backtest_hyperparams(
      regressor, X, target_series, hyperparams_list, backtest_start_date=None,
      backtest_end_date=None,
      forecast_type=False, meta_feature_prefix=None, meta_features=None, deploy_mode=True
):
    """
    ...
    maes = []
    wmapes = []
    for i, timestep in tqdm(enumerate(range(1, horizon + 1, 1)), desc=f"Getting backtest results for timesteps"):
        hyperparams = hyperparams_list[i]
        pm = pipeline_model.PipelineModel(hps=hyperparams, estimator=regressor, multiout_wrapper=False)
        ssf = rf.SingleTimestepRegressionForecaster(pm, timestep=timestep, forecast_type=forecast_type,
                                                    meta_feature_prefix=meta_feature_prefix)
        backtest_dates = X.index[(X.index >= backtest_start_date) & (X.index <= backtest_end_date)]
        predictions_list = []
...
def get_train_results(
      regressor, X, target_series, hyperparams_list,
      forecast_type=False, meta_feature_prefix=None, meta_features=None, deploy_mode=True
):
    """
    ...
    maes = []
    wmapes = []
    for i, timestep in tqdm(enumerate(range(1, horizon + 1, 1)), desc="Getting results on training set"):
        hyperparams = hyperparams_list[i]
        pm = pipeline_model.PipelineModel(hps=hyperparams, estimator=regressor, multiout_wrapper=False)
        ssf = rf.SingleTimestepRegressionForecaster(
            pm, timestep=timestep, forecast_type=forecast_type,
            meta_feature_prefix=meta_feature_prefix
        )
...
def metrics_frame_single(timestep, set_name, maes, rmses, mapes, wmapes):
    values = [maes[timestep - 1], rmses[timestep - 1], mapes[timestep - 1], wmapes[timestep - 1]]
    values_frame = pd.DataFrame(
        data=np.array(values).reshape((1, 4)), columns=["MAE", "RMSE", "MAPE", "WMAPE"], index=[set_name]
    )
    return values_frame
...
def main(df, target_name, horizon):
    ...
    meta_features = create_meta_feature_frame(df[target_name].dropna(), horizon, lead=24, ci_alpha=0.05)
    ...
    best_hyperparams_list = hyperopt_all_timesteps(
        regressor, horizon, X_train, target_series, space, cv,
        max_hyperopt_evals,
    ...
    forecast_table_test, rmses_test, mapes_test, maes_test, wmapes_test = backtest_hyperparams(
    ...
    forecast_table_val, rmses_val, mapes_val, maes_val, wmapes_val = backtest_hyperparams(
        regressor, df, target_series,
        best_hyperparams_list,
    ...
    values_frame_test = metrics_frame_single(1, "Test", rmses_test, mapes_test, maes_test, wmapes_test)
    ...
    feature_importances = model.forecasters[0].regressor.get_feature_importance(df.columns, [target_name])
