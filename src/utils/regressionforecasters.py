...
class MultipleTimestepRegressionForecaster(abc.ABC):
    ...
    def __init__(
          self, regressor, horizon, forecast_type="proportional_diff", meta_feature_prefix=None, recursion_strat=None,
            ...
            for timestep in self.timesteps:
                ...
                self.forecasters.append(
                    SingleTimestepRegressionForecaster(
                        copy.deepcopy(r), timestep, forecast_type=forecast_type,
                        meta_feature_prefix=meta_feature_prefix
                        )
                )
        else:
            if len(regressor) != len(self.timesteps):
                raise ValueError("If a list of regressors is given then its length must match the number of timesteps.")
            for i, t in enumerate(self.timesteps):
                r = regressor[i]
                self.forecasters.append(
                    SingleTimestepRegressionForecaster(
                        r, t, forecast_type=forecast_type,
                        meta_feature_prefix=meta_feature_prefix
                        )
                    )
    ...
    def __repr__(self):
        reprstring = f"MultipleTimestepRegressionForecaster(horizon={self._horizon}, " \
    ...
    def _fit_nonrecursive(self, X, target, freq, meta_features=None):
        ...
        for i, forecaster in enumerate(self.forecasters):
            self.forecasters[i] = forecaster.fit(X, target, freq=freq, meta_features=meta_features)
        return self
    ...
    def _fit_recursive(self, X, target, freq):
        ...
        for i, forecaster in enumerate(self.forecasters):
            ...
            recursive_column_names = [f"target_forecast_y{x}_t{i + 1}" for x in range(forecast.shape[1])]
            recursive_feature_last = pd.DataFrame(
                data=forecast.to_numpy(), index=X.index,
                columns=recursive_column_names
                )
            if self.recursion_strat == "recursive-single":
                ...
                recursive_features = pd.concat((recursive_features, recursive_feature_last), axis=1)
        return self
    ...
    def _predict_single(self, X=None, meta_features=None, conf_ints=False):
        ...
        """
        if X is None or self.recursion_strat is None:  # If no X is given as an argument the last set of inputs from the training data
            # will be used, which already include the recursive features, therefore the nonrecursive prediction method is called.
            forecasts = self._predict_single_nonrecursive(self, X=X, meta_features=meta_features, conf_ints=conf_ints)
        else:
    ...
    def _predict_single_nonrecursive(self, X=None, meta_features=None, conf_ints=False):
        ...
        for forecaster in self.forecasters:
            forecast = forecaster.predict(X, meta_features=meta_features, conf_ints=conf_ints)
    ...
    def _predict_single_recursive(self, X=None, meta_features=None, conf_ints=False):
        ...
        for i, forecaster in enumerate(self.forecasters):
            X_full = pd.concat((X, recursive_features), axis=1)
            forecast = forecaster.predict(X_full, meta_features=meta_features, conf_ints=conf_ints)
            forecasts.append(forecast)
            recursive_column_names = [f"target_forecast_y{x}_t{i + 1}" for x in range(forecast.shape[1])]
            recursive_feature_last = pd.DataFrame(
                data=forecast.to_numpy(), index=X.index,
    ...
    def predict(self, X=None, meta_features=None, conf_ints=False):
        ...
        if X is None or X.shape[0] == 1:
            forecast = self._predict_single(self, X=X, meta_features=meta_features, conf_ints=conf_ints)
            return forecast
        else:
            results_dict = {}
            for idx in X.index:
                ...
                if self._string_results_format:
                    ...
                    dict_idx = idx
                results_dict[dict_idx] = self._predict_single(self, X_single, meta_features=meta_features, conf_ints=conf_ints)
            return results_dict
...
class SingleTimestepRegressionForecaster(abc.ABC):
    ...
    def __init__(self, regressor, timestep, forecast_type="proportional_diff", meta_feature_prefix=None):
        ...
        if forecast_type == "absolute" and meta_feature_prefix is not None:
            raise ValueError("Can't have both absolute value prediction and meta feature boosting!")

    def __repr__(self):
        reprstring = f"SingleTimestepRegressionForecaster(timestep={self.timestep}, regressor={self.regressor}," \
                     f" forecast_type={self.forecast_type})"
        return reprstring
    ...
    def fit(self, X: pd.DataFrame, y, freq=None, meta_features=None, **kwargs):
        ...
        if y.index.freq is None:
            if freq is None:
                raise ValueError("Frequency must be provided as an argument if not in index of y")
            else:
                self.freq = freq
        else:
            if freq is not None and freq != y.index.freq:
                logging.error("Value of freq argument does not match freq of y, using freq of y instead")
        ...
        shifted_y = y.shift(-self.timestep).dropna()  # Dropna because shifting creates missing values
        if self.meta_feature_prefix is not None:
            ...
            meta_feature_mean = meta_features[self.meta_feature_prefix + f"_mean_t{self.timestep}"]
            # Align the meta feature with the y
            shifted_y, meta_feature_mean = shifted_y.align(meta_feature_mean, join="inner", axis=0)
            if self.forecast_type == "absolute_diff":
                shifted_y = shifted_y - meta_feature_mean.to_numpy().reshape(shifted_y.shape)
            ...
            if self.forecast_type == "absolute_diff":
                ...
                shifted_y = shifted_y / y.to_numpy()
        shifted_y, X = shifted_y.align(X, join="inner", axis=0)  # Reindex X to drop rows dropped in shifted_y
    ...
    def predict(self, X=None, y=None, meta_features=None, conf_ints=False):
        ...
        forecast = pd.DataFrame(data=forecast, index=forecast_index, columns=forecast_columns)
        return forecast
    ...
    def _get_conf_ints(self, mean_forecast=None, meta_features=None):
        ...
        meta_feature_cols = [self.meta_feature_prefix + s + f"_t{self.timestep}" for s in ("_mean", "_lower", "_upper")]
        # meta_features_current = meta_features[meta_feature_cols]
        cis_relative = meta_features[meta_feature_cols[1:]].to_numpy().reshape((-1, 2)) - meta_features[
            meta_feature_cols[0]].to_numpy().reshape((-1, 1))
...
class WrappedQuantileRegressor(abc.ABC):
    ...
    def __init__(self, regressor, quantiles, quantile_kw="alpha", objective_kwarg="quantile", **regressor_kwargs):
        """[summary]
