...
from utils.grange_and_correlate import (
    grange_and_correlate,
    user_input_correlation_picker,
)
...
def save_residual_frames(
    ...
    for timestep in range(1, horizon + 1, 1):
        ...
        resid_train.to_csv(
            f"BASF_Metals/Models/notebooks/outputs/residuals/residuals_train_t{timestep}.csv"
        )
        resid_val = add_residuals(forecast_tables_val[timestep - 1], target_series)
        resid_val.to_csv(
            f"BASF_Metals/Models/notebooks/outputs/residuals/residuals_val_t{timestep}.csv"
        )
        resid_test = add_residuals(forecast_tables_test[timestep - 1], target_series)
        resid_test.to_csv(
            f"BASF_Metals/Models/notebooks/outputs/residuals/residuals_test_t{timestep}.csv"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        forecast_type = (
            "absolute_diff"  # Either "proportional", "absolute_diff" or "absolute"
        )
        backtest_val_periods = 24  # Number of periods (e.g. months) used for backtesting and validation (each)
        validation_split_size = (
            2  # Number of validation periods in each validation split
        )
        ...
        #   CREATE TARGET FOLDERS
        # XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
        if os.path.exists("BASF_Metals/Models/notebooks/outputs"):
            shutil.rmtree(
                "BASF_Metals/Models/notebooks/outputs"
        ...
        with mlflow.start_run(run_name="single_timestep_run") as single_timestep_run:
            ...
            path2save_raw_data = os.path.join(
                artifact_location,
                "dataframes",
                target_name,
                f"horizon_{horizon}",
            ...
            mlflow.log_artifact(
                local_path=path2save_raw_data, artifact_path="forecasts"
            )
            ...
            path2save_df_dict = os.path.join(
                artifact_location,
                "dataframes",
                target_name,
                f"horizon_{horizon}",
            ...
            path2save_tidy_df = os.path.join(
                artifact_location,
                "dataframes",
                target_name,
                f"horizon_{horizon}",
            ...
            mlflow.log_artifact(
                local_path=path2save_tidy_df, artifact_path="forecasts/dataframe"
            )
            # XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
            #   SET META FEATURES
            # XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
            ...
            mlflow.log_artifact(
                local_path=path2save_meta_features, artifact_path="forecasts"
            )
            ...
            simulation_correlation_dict = user_input_correlation_picker(
                df[target],
            ...
            pre_selected_features, df_granger, df_correlation = grange_and_correlate(
                df, target, granger_lags=12, correlation_lags=12, number_of_features=20
            )
            ...
            df, target_series = feature_engineering_pipeline(
                df=df,
                target=target,
                horizon=horizon,
                pre_selected_features=indicator_cols,
            ...
            pre_selected_features_df_name = (
                f"{target_name}_pre_selected_features_df_{horizon}.pkl"
            )

            # The local paths to the dataframes
            dataframe_path = os.path.join(
                artifact_location, "dataframes", target_name, f"horizon_{horizon}"
            )
            ...
            path2save_pre_selected_features = os.path.join(
                dataframe_path, pre_selected_features_df_name
            )
            ...
            df[pre_selected_features].to_pickle(
                path2save_pre_selected_features, protocol=4
            )
            target_series.to_pickle(path2save_target, protocol=4)

            # Logging the artifacts in MlFlow
            mlflow.log_dict(
                simulation_correlation_dict,
                artifact_file="simulation/simulation_correlation_dict.json",
            )
            mlflow.log_artifact(
                local_path=path2save_granger, artifact_path="forecasts/dataframes"
            )
            mlflow.log_artifact(
                local_path=path2save_correlation, artifact_path="forecasts/dataframes"
            )
            mlflow.log_artifact(
                local_path=path2save_fe_df, artifact_path="forecasts/dataframes"
            )
            mlflow.log_artifact(
                local_path=path2save_target, artifact_path="forecasts/dataframes"
            )
            mlflow.log_artifact(
                local_path=path2save_pre_selected_features,
                artifact_path="forecasts/dataframes",
            )
            ...
            if backtest_val_periods % validation_split_size:
                raise ValueError(
                    "validation_split_size must be a factor of backtest_val_periods"
                )
            backtest_end_date = df.index[-1]
            backtest_start_date = df.index[-1] - df.index.freq * (
                backtest_val_periods - 1
            )
            validation_start_date = df.index[-1] - df.index.freq * (
                (backtest_val_periods * 2) - 1
            )
            ...
            space = {
                ...
                "sel__n_features_to_select": hyperopt.pyll.scope.int(
                    hp.quniform(
                        "sel__n_features_to_select", low=5, high=df.shape[1], q=1
                    )
                ),
                ...
                "est__n_estimators": hyperopt.pyll.scope.int(
                    hp.quniform(
                        "est__n_estimators",
                        low=4,
                        high=min(64, int(len(X_train) // 2)),
                        q=1,
                    )
                ),
                "est__num_leaves": hyperopt.pyll.scope.int(
                    hp.qloguniform(
                        "est__num_leaves", low=np.log(4), high=np.log(64), q=1
                    )
                ),  # max number of leaves in one tree
                "est__min_child_samples": hyperopt.pyll.scope.int(
                    hp.qloguniform(
                        "est__min_child_samples",
                        low=np.log(4),
                        high=np.log(2**4 + 1),
                        q=1,
                    )
                ),
                "est__colsample_bytree": hp.uniform(
                    "est__colsample_bytree", low=0.01, high=1.0
                ),
                ...
                "est__max_bin": hyperopt.pyll.scope.int(
                    hp.qloguniform("est__max_bin", low=np.log(7), high=np.log(128), q=1)
                ),
                "est__lambda_l1": hp.loguniform(
                    "est__lambda_l1", low=np.log(1.0 / 16), high=np.log(16)
                ),
                "est__lambda_l2": hp.loguniform(
                    "est__lambda_l2", low=np.log(1.0 / 16), high=np.log(16)
                ),
                "est__learning_rate": hp.loguniform(
                    "est__learning_rate", low=np.log(0.001), high=np.log(1.0)
                ),
                # 'est__subsample': hp.uniform('est__subsample', low=0.3, high=1.0),
                "est__bagging_fraction": hp.uniform(
                    "est__bagging_fraction", low=0.01, high=1
                ),  # alias "subsample
                ...
                "est__max_depth": hyperopt.pyll.scope.int(
                    hp.quniform("est__max_depth", low=2, high=32, q=1)
                ),
                # 'est__bagging_fraction': hp.uniform('est__bagging_fraction', low=0.01, high=1), #alias "subsample"
                "est__feature_fraction": hp.uniform(
                    "est__feature_fraction", low=0.01, high=1
                ),
            ...
            values_frame_test = metrics_frame_single(
                1, "Test", rmses, mapes, maes, wmapes
            )
            ...
            mlflow.log_dict(
                {"index": [i.strftime("%Y-%m") for i in df.index]}, "index.json"
            )
            ...
            with mlflow.start_run(
                run_name="MultipleTimestepRegressionForecaster", nested=True
            ) as MultipleTimestepRegressionForecaster:
                pipeline_list = [
                    pipeline_model.PipelineModel(
                        hps=h, estimator=regressor, multiout_wrapper=False
                    )
                ...
                mlflow.sklearn.log_model(
                    sk_model=model,
                    registered_model_name="Multiple_Timestep_Regression_Forecaster_unfit",
                    artifact_path=os.path.join("BASF_Metals", "artifacts", "Model"),
                )
                ...
                forecast = pd.concat((target_series, forecast), axis=0).drop(
                    columns=target
                )
                model_uri = "runs:/{}/sklearn-model".format(
                    MultipleTimestepRegressionForecaster.info.run_id
                )
                import pdb
                ...
                # Timestep 1
                feature_importances_1 = model.forecasters[
                    0
                ].regressor.get_feature_importance(df.columns, [target])
                feature_importances_1 = feature_importances_1.rename(
                    columns={"y": "importance"}
                )
                feature_importances_1.index.name = "feature"
                # Timestep 2
                feature_importances_2 = model.forecasters[
                    1
                ].regressor.get_feature_importance(df.columns, [target])
                feature_importances_2 = feature_importances_2.rename(
                    columns={"y": "importance"}
                )
                feature_importances_2.index.name = "feature"
                # Timestep 3
                feature_importances_3 = model.forecasters[
                    2
                ].regressor.get_feature_importance(df.columns, [target])
                feature_importances_3 = feature_importances_3.rename(
                    columns={"y": "importance"}
                )
                ...
                feature_importances_1.to_csv(
                    os.path.join(
                        ...
                        "forecast",
                        "Feature_importance",
                        "feature_importances_1.csv",
                    )
                )
                feature_importances_2.to_csv(
                    os.path.join(
                        base_path,
                        "BASF_Metals",
                        "artifacts",
                        ...
                        "forecast",
                        "Feature_importance",
                        "feature_importances_2.csv",
                    )
                )
                feature_importances_3.to_csv(
                    os.path.join(
                        base_path,
                        "BASF_Metals",
                        "artifacts",
                ...
                mlflow.log_artifacts(
                    ...
                    artifact_path="forecasts/results_run",
                )
