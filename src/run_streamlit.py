...
def exploratory_data_analysis(data):
    ...
    option = st.sidebar.selectbox(
        ...
        ['Time Series', 'Cross-Correlation', 'Box Plot', 'Granger Casaulty check'], key = 'EDA_type')
...
def plot_historical_forecast(chart, commodity):
    st.write(f"### The plot for {st.session_state.historical_func_type} of {commodity} ")
...
def historical_forecast():
    ...
    if st.session_state.Backtesting == True:
        ...
        st.plotly_chart(backtesting_chart, use_container_width=False, sharing="streamlit")
        st.session_state.Backtesting = False

# XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
#   Simulation
# XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX 
def plot_simulation(simulation_fig, forecast, original_forecast, horizon, commodity, data = None, final_simulation_correlation_df=None):
    diff = forecast.reset_index()[['forecast','lower','upper']].sub(original_forecast.reset_index()[['forecast','lower','upper']])
...
def simulation(data):
    ...
    with simulation_options:
        ...
        all_features = data.columns
        num_sim_feat = st.sidebar.number_input(label = 'Please select the number of features to simulate',min_value=1,max_value=len(all_features),key = 'num_sim_feat' )
        horizon = st.sidebar.number_input(label = 'Please select the number of lookahead months to simulate',min_value=1,max_value=3,key = 'horizon' )
        ...
        simulated = st.form_submit_button('Simulate', on_click=set_simulation_dict, args = (simulation_dict,))  
    ...
    if simulated:
        ...
        # with st.spinner(text="Simulating the future"):
            simulation_fig, forecast, original_forecast, final_simulation_correlation_df = simulate.main(simulation_dict = st.session_state.simulation_dict, target = commodity, sim_df= data, horizon=horizon)
        plotiing = plot_simulation(simulation_fig, forecast, original_forecast, horizon, commodity, data, final_simulation_correlation_df)
...
def main():
    ...
    names = ['BASF test 1','BASF test2']
    usernames = ['BASFtest1','BASFtest2']
    passwords = ['123','456']
