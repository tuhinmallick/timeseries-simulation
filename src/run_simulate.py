...
def features_picker(simulation_dict, sim_df, target,original_target, horizon):
    ...
    meta_simulation_correlation = grange_and_correlate.user_input_correlation_picker(meta_features_df[key], pre_selected_features_df, selected_method='max', max_lags=12)
    for feature_name, correlation in meta_simulation_correlation.items():
                meta_change_pre_selected_df[feature_name]= meta_features_df[feature_name] * correlation * (value/100)
                modified_meta_features_df[feature_name]= meta_features_df[feature_name] + meta_change_pre_selected_df[feature_name]
    for key, value in simulation_dict.items():
        ...
        pretty_simulation_correlation=feature_renaming.rename_correlation_dict_features(simulation_correlation, df_dict)
        pretty_simulation_correlation_df = pd.DataFrame(pretty_simulation_correlation, index=[1]).melt()
        pretty_simulation_correlation_df.columns = ["Feature Name", "Correlation fraction for {}".format(key)]
        if first_iteration:
    ...
    return modified_pre_selected_df, modified_meta_features_df, target_series_df, final_simulation_correlation_df
...
def main(simulation_dict : dict, sim_df : pd.DataFrame, target: str, horizon: int):
    ...
    forecast_fig = plotly_visualization.plotly_plot_simulation( df_actuals = target_series.rename(columns= {target_name:'actual'}),df_forecast=original_forecast,  df_simulation = forecast, display_fig=False, figsize=(1400, 500))
