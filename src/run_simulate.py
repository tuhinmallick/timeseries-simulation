...
def features_picker(simulation_dict, sim_df, target, original_target, horizon):
    modified_pre_selected_df, simulation_correlation, change_pre_selected_df = (
        None,
        {},
        None,
    )
    (
        meta_change_pre_selected_df,
        modified_meta_features_df,
        meta_simulation_correlation,
    ) = (None, None, {})
    # subscription_id = 'fc563dde-a9ad-4edf-9998-2d52ba8afff9'
    # # Azure Machine Learning resource group
    # resource_group = 'forecasty_ml_test'
    ...
    dataframe_path = os.path.join(
        artifact_location, "dataframes", original_target, f"horizon_{horizon}"
    )
    ...
    pre_selected_features_df_name = (
        f"{original_target}_pre_selected_features_df_{horizon}.pkl"
    )
    ...
    df_dict = pd.read_csv(
        os.path.join(artifact_location, "source_data", "PGM_data_dictionary.csv")
    ...
    meta_features_df = make_meta_features.create_meta_feature_frame(
        df_tidy[target].dropna(), horizon, ci_alpha=0.05
    )
    ...
    for key, value in simulation_dict.items():
        meta_simulation_correlation = (
            grange_and_correlate.user_input_correlation_picker(
                sim_df[key], meta_features_df, selected_method="max", max_lags=12
            )
        )
        for feature_name, correlation in meta_simulation_correlation.items():
            meta_change_pre_selected_df[feature_name] = (
                meta_features_df[feature_name] * correlation * (value / 100)
            )
            modified_meta_features_df[feature_name] = (
                meta_features_df[feature_name]
                + meta_change_pre_selected_df[feature_name]
            )
    for key, value in simulation_dict.items():
        simulation_correlation = grange_and_correlate.user_input_correlation_picker(
            sim_df[key], pre_selected_features_df, selected_method="max", max_lags=12
        )
        simulation_correlation_df = pd.DataFrame(
            simulation_correlation, index=[1]
        ).melt()
        ...
        for feature_name, correlation in simulation_correlation.items():
            change_pre_selected_df[feature_name] = (
                pre_selected_features_df[feature_name] * correlation * (value / 100)
            )
            modified_pre_selected_df[feature_name] = (
                pre_selected_features_df[feature_name]
                + change_pre_selected_df[feature_name]
            )

        # modified_pre_selected_df = modified_pre_selected_df.append(target_series_df, ignore_index=False)
        modified_pre_selected_df = pd.concat(
            [modified_pre_selected_df, target_series_df], axis=1
        )
        pretty_simulation_correlation = (
            feature_renaming.rename_correlation_dict_features(
                simulation_correlation, df_dict
            )
        )
        pretty_simulation_correlation_df = pd.DataFrame(
            pretty_simulation_correlation, index=[1]
        ).melt()
    ...
    modified_pre_selected_df, _ = smb_feature_engineering.feature_engineering_pipeline(
        modified_pre_selected_df,
        target,
        horizon,
        pre_selected_features_columns,
        forecast_type="absolute_diff",
    )
    print(
...
def load_model(horizon, target, local=True):
    if local:
        ...
        with open(
            os.path.join(
                artifact_location, "model", target, f"horizon_{horizon}", _model_name
            ),
            "rb",
        ) as meta_features:
...
def main(simulation_dict: dict, sim_df: pd.DataFrame, target: str, horizon: int):
    ...
    forecast = model.predict(
        X=modified_pre_selected_df.iloc[-1:],
    ...
    forecast_fig = plotly_visualization.plotly_plot_simulation(
        df_actuals=target_series.rename(columns={target_name: "actual"}),
        df_forecast=original_forecast,
        df_simulation=forecast,
        display_fig=False,
        figsize=(1400, 500),
    )
