...
artifact_location = os.path.join(pathlib.Path(__file__).absolute().parent.parent.parent, 'artifacts')
...
def historical_forecast():
    ...
    for items in forecast_list:
        ...
        with open(os.path.join(artifact_location,'forecast',items,pickle_df), 'rb') as _file:
    ...
    if  forecast.button('Forecast', on_click=login.set_Forecast):
        ...
        st.plotly_chart(item_dict.get('forecast_chart'), use_container_width=False, sharing="streamlit")
    ...
    if  feature_importance.button('Drivers', on_click=login.set_Drivers):
        ...
        st.plotly_chart(item_dict.get('forecast_drivers'), use_container_width=False, sharing="streamlit")
    ...
    if backtesting.button('Backtesting', on_click=login.set_Backtesting):
        ...
        st.plotly_chart(item_dict.get('forecast_backtesting'), use_container_width=False, sharing="streamlit")
